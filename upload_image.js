...
export default class UploadImage {
  ...
  constructor(container, Croppie) {
    // set the value of container;
    this.container = container;

    const UploadImage = this;

    //put image UI inside the container
    this.container.innerHTML = `
          <div style="display: none" class=" c_ui_crop_state crop-state">
            <div class="c_ui_crop"></div>
            <div class="crop-state__buttons">
              <a class=" c_ui_cancel crop-state__btn ">CANCEL</a>
              <a class=" c_ui_save crop-state__btn ">SAVE</a>
            </div>
          </div>

          <div class=" c_ui_select_state select-state">
            <img style="display: none" class="c_ui_image_circle" dataSrc='' src=""/>
            <svg class=" c_ui_select select-state__btn"><use xlink:href="plus.svg#Capa_1"></use></svg>
          </div>

          <div style="display: none;">
            <input class="c_ui_file_input" type="file" name="upload_image" accept="image/*">
          </div>
      `;

    //state - views
    this.selectState = this.container.querySelector('.c_ui_select_state'); // when the user has to click the button to choose an image
    this.cropState = this.container.querySelector('.c_ui_crop_state'); // croppie state (the user should crop the image)
    this.cropState.style.display = 'none'; // hide the croppie state first

    //buttons
    const fileInput = this.container.querySelector('.c_ui_file_input'); // the hidden input button
    const selectImage = this.container.querySelector('.c_ui_select'); // camera icon to add image
    const cancelCrop = this.container.querySelector('.c_ui_cancel'); // cancel button in the crop state
    const saveCrop = this.container.querySelector('.c_ui_save'); // save button in the crop state


    const imageCircle = this.container.querySelector('.c_ui_image_circle'); // the circle in which the cropped image goes
    imageCircle.style.display = 'none'; // should be hidden by default
    const croppieElement = this.container.querySelector('.c_ui_crop'); // the element in which croppie will put the crop ui

    let vanilla; // an instance of a croppie class

    if (window.screen.width < 370) { // size of croppie ui when the screen is very small
      vanilla = new Croppie(croppieElement, {
        enableExif: true,
        viewport: {width: 190, height: 190, type: 'square'},
        boundary: {width: 280, height: 220}
      })
    } else if (window.screen.width < 420) { // size of croppie ui when the screen is mobile size
      vanilla = new Croppie(croppieElement, {
        enableExif: true,
        viewport: {width: 190, height: 190, type: 'square'},
      ...
      vanilla = new Croppie(croppieElement, { // size of croppie ui when the screen is larger than mobile
        enableExif: true,
        viewport: {width: 170, height: 170, type: 'square'},
        boundary: {width: 410, height: 220}
      })
    }

    /**
     * SELECT IMAGE
     * when the add image button is clicked, click the file input
     */
     ...
     * when an image has been selected, hide the select state
     */
    fileInput.addEventListener("change", function() {

      UploadImage.showCropState(); // display the crop state


      const reader = new FileReader();

      // this happens when the image is loaded
      reader.onload = function(e){
        // e - the progressEvent
        // e.target.result - the base 64 of the image
        vanilla.bind({
          url: e.target.result
        }).then(function(){
          // do something after binding the image to croppie
        });
      };

      //this happens on loadend
      // if a file was selected, get it
      if(this.files[0]){
        reader.readAsDataURL(this.files[0]);
      }else{
        //do something if a file is not selected
      }
    }); //when an image has been selected
    ...
    saveCrop.addEventListener('click',() => {
      ...
      }).then(function(response){ //response is a base 64 version of the image

        // set the background of the select state to the cropped image
        UploadImage.selectState.style.background = `url('${response}')`;
        UploadImage.showSelectState(); // display the select state

        //create
        /* change the source of the image element to the cropped image.
          we only need this to diaplay the image
          */
        imageCircle.src = response;
        /*
         change the dataSrc attribute of the image to the cropped image
         this is highly important because the form will get the image from the data src
          */
        imageCircle.attributes.dataSrc.value = response;
        /*
         display the image
         even if it is sent to none, the image will still be displayed because
         the background of the select state is the cropped image
          */
        imageCircle.style.display = 'block';

        UploadImage.setImage(response); //set class image property to the cropped image
      })
    }); // when the save button is selected
    ...
    cancelCrop.addEventListener( 'click', () => {
      UploadImage.showSelectState();
      // UploadImage.cropState.style.display = 'none'; // hide the crop state
      // UploadImage.selectState.style.display = 'block'; // display the select state
    });
  ...
  showCropState(){
    this.selectState.style.display = 'none'; //hide the select state
    this.cropState.style.display = 'block'; //display the crop state
  }

  showSelectState(){
    this.cropState.style.display = 'none'; // hide the crop state
    this.selectState.style.display = 'block'; // display the select state
  }
}